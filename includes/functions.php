...
function tlr_get_number_info( string $mobile ): array
{
	...
	try {
		...
		$data['intl_number'] = $phone_util->format($number_proto, \libphonenumber\PhoneNumberFormat::INTERNATIONAL);
		$data['national_number'] = $phone_util->format($number_proto, \libphonenumber\PhoneNumberFormat::NATIONAL);
		$data['region_code'] = $phone_util->getRegionCodeForNumber($number_proto);
		$data['country_code'] = $phone_util->getCountryCodeForRegion($data['region_code']);
		$data['type'] = isset($number_types[$phone_util->getNumberType($number_proto)]) ? $number_types[$phone_util->getNumberType($number_proto)] : $number_types[10];

		$geo_coder = \libphonenumber\geocoding\PhoneNumberOfflineGeocoder::getInstance();
		$data['area_desc'] =  $geo_coder->getDescriptionForNumber($number_proto, "en_US"); //todo translate and change locale

		$carrier_mapper = \libphonenumber\PhoneNumberToCarrierMapper::getInstance();
		$data['carrier'] = $carrier_mapper->getNameForNumber($number_proto, "en");  //todo translate and change locale

		$timeZoneMapper = \libphonenumber\PhoneNumberToTimeZonesMapper::getInstance();
		$data['time_zones'] = $timeZoneMapper->getTimeZonesForNumber($number_proto);
...
function tlr_convert_numbers( string $text, string $dest_lang = null ): string
{
	...
	if ( isset( $save_links ) ) {
		$text = str_replace( array_keys($save_links), array_values($save_links), $text );
	}
	return $text;
}
...
function tlr_sanitize_input( $field_value, $field_id = null, $mode = 'user_reg' )
{
	switch ( $field_id ) {
		...
		case 'member_group':
			...
			foreach ( $field_value as $key => $member_group ) {
				...
				if ( 'admin_edit' === $mode && $member_group > 0 )  {
					$term = get_term_by('id', (int) $member_group, 'member_group' );
				} else {
		...
		case 'gateway':
			...
			return in_array( $field_value, $active_gateways, true ) ? $field_value : '';
			break;
		case 'first_name':
		case 'last_name':
		case 'title':
 ...
 */
function tlr_validate_field( $field_id, $field_data, $field_value )
{
	...
	return tlr_is_field_value_valid($field_id, $field_value) ? 10 /* valid */ : -1 /* not valid */;
}
 ...
 * @return bool|object
 */
function tlr_member_exists( $member_mid, $field = 'mobile')
{
	$member_id = tlr_get_member_id($member_mid, $field);
	return $member_id > 0 ? true : false;
}
...
function tlr_get_user_id( $mid, $field = 'ID' )
{
	//todo: sanitize $field
	global $wpdb;
	$table_name = $wpdb->prefix . 'tlr_members';
...
function tlr_get_possible_user( $maybe_mobile, $default_countries )
{
	...
	if ( !empty($mobile) ) {
		...
		$user = tlr_get_user_by_mobile( $mobile );
		return $user;
...
function tlr_get_mobile( $mid, $field = 'ID' )
{
...
function tlr_member_query ( $search )
{
	global $wpdb;
...
function tlr_get_name( $post_id )
{
	// todo
	$fname = get_post_meta( $post_id, '_tlr_first_name', true );
	$lname = get_post_meta( $post_id, '_tlr_last_name', true );
	...
	if ( !empty($fname) && !empty($lname) ) {
		...
		return tlr_get_mobile($post_id, 'post_id');
	}
}
...
function tlr_date_difference($newDateTime , $oldDateTime , $differenceFormat = 'm' )
{
	try {
		$newDateTime = new \DateTime($newDateTime);
		$oldDateTime   = new \DateTime($oldDateTime);
	} catch (\Exception $e) {
		tlr_write_log($e->getMessage());
		return false;
	}
	$interval  = $newDateTime->diff($oldDateTime);
	$diff = $interval->format('%h:%i:%s');
	$diff = explode(':',$diff);
	if ( ! $diff ) {
		return false;
	}
	$diff = array_combine( ['h', 'm', 's'], $diff );


	switch ($differenceFormat) {
		case 'm':
		default:
			$minutes = (int)$diff['h'] * 60 + (int)$diff['m'];
			return $minutes;
	}
}
 ...
 * @return mixed
 */
function tlr_get_var( &$var, $default = null )
{
...
function tlr_get_posted_field_value( $field_id, $member = null, $custom_name = '' )
{
	$getter_method = "get_$field_id";
	$stored_value = ! is_null( $member ) && method_exists( $member, $getter_method ) ? $member->$getter_method() : '';
	...
	switch ( $field_id ) {
		...
		case 'email':
			if( $custom_name ) {
				$posted_value = ! empty( $_POST[$custom_name] ) ? esc_attr( wp_unslash( $_POST[$custom_name] ) ) : '';
			} else {
				$posted_value = ! empty( $_POST["tlr_$field_id"] ) ? esc_attr( wp_unslash( $_POST["tlr_$field_id"] ) ) : '';
			}
			break;
		case 'description':
			if( $custom_name ) {
				$posted_value = ! empty( $_POST[$custom_name] ) ? esc_textarea( wp_unslash( $_POST[$custom_name] ) ) : '';
			} else {
				$posted_value = ! empty( $_POST["tlr_$field_id"] ) ? esc_textarea( wp_unslash( $_POST["tlr_$field_id"] ) ) : '';
			}
			break;
...
function tlr_get_public_member_groups() : array
{
	...
	return !is_wp_error($member_group) ? wp_list_pluck($member_group, 'name', 'slug') : [];
}
...
function tlr_add_notice( \WP_Error &$errors, $error_code, $error_msg, $notice_title = true )
{
	if ( $notice_title ) {
...
function tlr_get_base_tags_values_array( string $base_tag_id, $object ) : array
{
	...
	if ( 'member' === $base_tag_id ) {
		...
		$statuses = [
			'canceled'      =>  _x( 'canceled', 'replacement text for the {status} tag', 'texteller' ),
			'registered'    =>  _x( 'registered','replacement text for the {status} tag', 'texteller' ),
			'verified'      =>  _x( 'verified', 'replacement text for the {status} tag', 'texteller' )
		];

		// set status for draft members (not-saved yet)
		$member_status = $object->get_status();
		$status = !empty($member_status) && isset($statuses[$member_status]) ? $statuses[$member_status] : '';

		switch ($object->get_title()) {
			case 'Mrs':
				$title = _x( 'Mrs','replacement text for the {title} tag', 'texteller' );
				break;
			case 'miss':
				$title = _x( 'Miss','replacement text for the {title} tag', 'texteller' );
				break;
			case 'ms':
				$title = __( 'Ms','replacement text for the {title} tag', 'texteller' );
				break;
			case 'mr':
				$title = __( 'Mr','replacement text for the {title} tag', 'texteller' );
				break;
			default:
				$title = '';
		}
		...
		$member_group_text = [];
		$member_group = $object->get_member_group();
		foreach ( (array) $member_group as $member_group) {
			//if($member_group === 'tlr_newsletter') continue;
			$term = get_term_by('slug', $member_group, 'member_group');
			if (is_object($term)) {
		...
		$base_tags_values = [
			'member_first_name'     =>  ! empty($object->get_first_name()) ? $object->get_first_name() : '',
			'member_last_name'      =>  ! empty($object->get_last_name()) ? $object->get_last_name() : '',
			'member_full_name'      =>  ! empty($object->get_name()) ? $object->get_name() : '',
			'member_reg_date'       =>  str_replace('-','/', $object->get_registered_date()),  //todo: settings to choose date time format
			'member_title'          =>  $title,
			'member_member_group'   =>  $member_group_text,
			'member_status'         =>  $status,
			'member_email'          =>  ! empty($object->get_email()) ? $object->get_email() : '',
...
function tlr_sanitize_text_field_array( array $values )
{
...
function tlr_maybe_explode( string $string, string $delimiter = ',' ) : array
{
 ...
 */
function tlr_get_gateway_class( $gateway )
{
	if ( empty ($gateway) ) {
		return false;
	}
     ...
     */
    $gateway_class = apply_filters('tlr_gateway_class_name', $gateway_class, $gateway);
...
function get_notification_triggers()
{
	...
	$base_triggers = [
		'tlr_manual_send'        =>  _x( 'Texteller: Admin Manual Send', 'message trigger', 'texteller' ),
		'tlr_manual_send_member' =>  _x( 'Texteller: Member Manual Send', 'message trigger', 'texteller' ),
		'tlr_inbound_message'    =>  _x( 'Inbound Message', 'message trigger', 'texteller' )
	];
	...
	foreach ( $wp_settings_fields as $page_slug => $page_options ) {
		...
		foreach ( $page_options as $section_slug => $section_options ) {
			foreach ( $section_options as $option_id => $option_data  ) {
				if ( !isset($option_data['args']['type']) || 'notification' !== $option_data['args']['type'] ) {
					continue;
				}
				...
				$notification_triggers[$trigger_slug] = $option_data['args']['title'];
			}
		}
	}
...
function get_page_sections( string $page ) : array
{
	...
	global $wp_settings_pages;
	$a = $wp_settings_pages;
	if ( isset($wp_settings_sections[$page]) ) {
		$sections = [];
		foreach ( $wp_settings_sections[ $page ] as $section_slug => $section_data ) {
...
function get_form_fields( string $option_name, string $option_class )
{
	$fields = [];

	if ( class_exists($option_class) && method_exists($option_class,'get_default_fields') ) {
		$defaults = $option_class::get_default_fields();
		$stored_fields = get_option( $option_name, [] );

		if ( !empty($stored_fields) ) {
			$field_slugs = array_keys( array_merge( array_flip(array_keys($stored_fields)), $defaults ) );
		} else {
		...
		foreach( $field_slugs as $field_slug ) {
			...
			$fields[$field_slug] = !empty($stored_fields[$field_slug])
				? $stored_fields[$field_slug] : $defaults[$field_slug];

			// If we don't have a stored title, use default value
			$fields[$field_slug]['title'] = !empty($stored_fields[$field_slug]['title'])
				? $stored_fields[$field_slug]['title'] : $defaults[$field_slug]['title'];

			// If $has_size_field but we don't have a stored size, use default value
			if ( empty($stored_fields[$field_slug]['size']) && !empty($defaults[$field_slug]['size']) ) {
 ...
 * @param array $vars Additional data to be passed to the template
 * @since 0.1.3
 */
function get_admin_template( $template, $vars = [] )
{
