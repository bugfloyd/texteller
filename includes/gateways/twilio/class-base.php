...
class Base implements TLR\Interfaces\Gateway
{
	...
	/**
	 * @var null|\Twilio\Rest\Client $client Twilio Client
	 */
	private static $client = null;

	/**
	 * @var null|\Twilio\Rest\Api\V2010\AccountInstance $account
	 */
	private static $account = null;

	public function __construct()
    {
	    require_once( TLR_INC_PATH . '/gateways/twilio/lib/autoload.php' );
    }
	...
	public static function get_account()
	{
		...
		try {
			self::$account = self::$client->api->v2010->accounts( self::$client->getAccountSid() )->fetch();
			return self::$account;
		} catch ( \Twilio\Exceptions\TwilioException $e ) {
			TLR\tlr_write_log( 'Twilio: An error occurred. | ' . $e->getMessage() );
			return false;
		}
	}
	...
	public static function get_incoming_phone_numbers()
	{
		...
		if ( empty($account->subresourceUris['incoming_phone_numbers']) ) {
			TLR\tlr_write_log('Twilio: Cannot get account incoming_phone_numbers subsource URI');
		} else {
			$numbersUrl = 'https://api.twilio.com' . $account->subresourceUris['incoming_phone_numbers'];
			...
			if ( !isset($responseContent['incoming_phone_numbers']) ) {
				TLR\tlr_write_log('Twilio: Cannot get incoming_phone_numbers details');
			} else {
	...
	public static function get_account_details()
	{
		...
		$date_created = empty($account->dateCreated)? $account->dateCreated : '';
		...
		$friendly_name = empty($account->friendlyName)? $account->friendlyName : '';
		$status = empty($account->status)? $account->status : '';
		$type = empty($account->type)? $account->type : '';

		$balance = '';
		if ( empty($account->subresourceUris['balance']) ) {
			TLR\tlr_write_log('Twilio: Cannot get account balance subsource URI');
		} else {
			...
			if ( !isset($responseContent['balance']) || !isset($responseContent['currency']) ) {
				TLR\tlr_write_log('Twilio: Cannot get account balance details');
			} else {
				$balance = round($responseContent['balance'], 2) . ' ' . $responseContent['currency'];
			}
		}
		return [
	...
	public static function get_client()
	{
		...
		try {
			self::$client = new \Twilio\Rest\Client( $sid, $token );
			return self::$client;
		}
		catch ( \Twilio\Exceptions\ConfigurationException $e){
			TLR\tlr_write_log( 'Twilio authentication failed. | ' .$e->getCode() );
			return false;
		}
	}
	...
	private function send_sms( string $text, string $number )
	{
		...
		try {
			$MessageInstance = self::$client->messages->create(
				$number,
				[
					'from'              =>  get_option( 'tlr_gateway_twilio_number' ),
					'body'              =>  $text,
					'statusCallback'    =>  get_rest_url(null, 'texteller/v1/delivery/twilio/')
				]
			);
	 ...
	 */
	public static function rest_delivery_callback( $request )
    {
	    ...
	    if (
                ...
                && in_array($delivery_body['MessageStatus'], [ 'sent','delivered','read','failed' ], true)
        ) {
	        $args = [
	                'object_type'   =>  'message',
                    ...
                    'gateways'      =>  ['twilio'],
                    'field'         =>  'ID',
                    'gateway_data'  =>  sanitize_text_field( $delivery_body['MessageSid'] )
            ];
	...
	public static function rest_receive_callback( $request )
	{
		...
		$message_sid = isset($message_body['MessageSid']) ? sanitize_text_field($message_body['MessageSid']) : '';
		$text = isset($message_body['Body']) ? sanitize_text_field($message_body['Body']) : '';
		$to = isset($message_body['To']) ? sanitize_text_field($message_body['To']) : '';
		$from = isset($message_body['From']) ? sanitize_text_field($message_body['From']) : '';
		...
		if ( 'POST' !== $method || !$from || ! $to || !$message_sid) {
			...
			if ($validator->validate($request->get_header('x-twilio-signature'), $url, $message_body)) {
				...
				return new \WP_Error( 'rest_forbidden_request', esc_html('Forbidden'), [ 'status' => 403 ] );
			}
		}
	}
	...
	{
        self::add_options();
		add_action( 'tlr_options_after_fields', [ self::class, 'render_gateway_status' ], 10, 2 );
		add_filter( 'pre_update_option_tlr_gateway_twilio_auth_token', [self::class, 'update_encrypted_option'], 10, 2);
		add_filter( 'pre_update_option_tlr_gateway_twilio_number', [self::class, 'initialize_phone_numbers'] );
	}
	...
	public static function add_options()
	{
		self::register_section([
			...
			'desc'  =>  sprintf( /* translators: %s: Gateway name */
				__( 'Configure %s gateway options', 'texteller' ), __('Twilio', 'texteller')
			),
			'class' =>  'description',
			'page' =>  'tlr_gateways'
		]);
		...
		$options = [
			...
			[
				...
				'page'      =>  'tlr_gateways',
				'section'   =>  'tlr_gateway_twilio',
				'desc'          =>  'Twilio account Auth Token acquired from twilio.com/console',
				'type'          =>  'input',
				'params'    =>  [
					'type'  =>  'password'
				]
			...
			[
				...
				'page'      =>  'tlr_gateways',
				'section'   =>  'tlr_gateway_twilio',
				'desc'      =>  'The Twilio phone number purchased from twilio.com/console',
			...
			if ( !$numbers ) {
				...
				foreach ( (array) $numbers as $number ) {
					if ( !empty($number['phone_number']) ) {
						?>
						<option value="<?= esc_attr($number['phone_number']) ?>"<?php selected( $stored_value,$number['phone_number'] ) ?>><?= esc_html($number['friendly_name']) ?></option>
						<?php
					}
				}
			}
	...
	{
		if ( 'tlr_gateway_twilio' !== $current_section || 'tlr_gateways' !== $current_tab ) {
			return;
		}
        ...
        <div class="gateway-info-wrap">
			...
			if( $account_details ) {
				?>
                <div class="gateway-info-label-wrap">
                    <span><?= esc_html__( 'Account Friendly Name', 'texteller' ) ?></span>
                </div>
                <div class="gateway-info-value-wrap">
                    <span><?= esc_html($account_details['friendly_name']) ?></span>
                </div>
                <div class="gateway-info-label-wrap">
                    <span><?= esc_html__( 'Date Created', 'texteller' ) ?></span>
                </div>
                <div class="gateway-info-value-wrap">
                    <span><?= esc_html($account_details['date_created']) ?></span>
                </div>
                <div class="gateway-info-label-wrap">
                    <span><?= esc_html__( 'Account Status', 'texteller' ) ?></span>
                </div>
                <div class="gateway-info-value-wrap">
                ...
                <div class="gateway-info-value-wrap">
                <span><?= esc_html( get_rest_url(null, 'texteller/v1/receive/twilio/') ) ?></span>
                </div><?php
			}
			?>
        </div>
