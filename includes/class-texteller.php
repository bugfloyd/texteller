...
* Class Texteller
*
* Main Texteller class, add filters and handling all other files
*
* @class		Texteller
* @version		1.0.0
...
final class Texteller
{
	...
	* @access protected
	* @var object $instance The instance of Texteller.
	*/
	...
	public function __wakeup() {
		_doing_it_wrong( __FUNCTION__, 'Unserializing instances of this class is forbidden.', '3.0' );
	}
	...
	*/
	private function __construct()
	{
		...
		add_filter( 'plugin_action_links_' . plugin_basename(TLR_PLUGIN_FILE), [self::class, 'plugin_action_links'] );
		add_filter( 'plugin_row_meta', [self::class, 'plugin_desc_links' ], 10, 2 );

		$this->includes();

		add_action( 'wp_enqueue_scripts', [self::class, 'register_mobile_field_assets'] );
	 ...
	 * @return array
	 */
	public static function plugin_desc_links( $links, $file )
	{
		if ( plugin_basename(TLR_PLUGIN_FILE) === $file ) {
			$row_links = [
				'website'    => '<a href="' . esc_url( 'https://www.texteller.com/' ) . '" aria-label="' . esc_attr__( 'View Texteller official website', 'texteller' ) . '">' . esc_html__( 'Official Website', 'texteller' ) . '</a>'
			];
	...
	{
		$action_links = [
			'settings' => '<a href="' . admin_url( 'admin.php?page=tlr-options' ) . '" aria-label="' . esc_attr__( 'Texteller Options', 'texteller' ) . '">' . esc_html__( 'Options', 'texteller' ) . '</a>',
		];
		return array_merge($action_links, $links);
	}

	public static function register_mobile_field_assets()
	{
		// Intl Tel Input
		wp_register_script(
			'tlr-intl-tel-input',
	...
	*/
	...
	public static function autoload_libphonenumber()
	{
	...
	public static function install()
	{
		...
		if ( true !== $compatibility_status ) {
			...
			if ( $compatibility_status === -1 ) {
				...
				die( __( 'Texteller requires WordPress 5.3 or higher.', 'texteller' ) );
			}
		} else {
			self::create_tables();
			self::create_files();
	...
	private static function create_files()
	{
		...
		// Write the file
		$css = $wp_filesystem->get_contents( TLR_ASSETS_PATH . '/newsletter/tlr-newsletter.css' );
		if ( !empty($css) ) {
    ...
    {
        add_action( 'tlr_cron_send', [ 'Texteller\Admin\Manual_Send', 'cron_send' ], 10, 5 );
        add_action( 'tlr_cron_import', [ 'Texteller\Admin\Tools', 'cron_import' ], 10, 2 );
    }
	...
	protected function define_constants()
	{
		...
		if ( ! defined( 'TLR_MODULES_URI' ) ) {
			define( 'TLR_MODULES_URI', plugin_dir_url(TLR_PLUGIN_FILE) . 'modules' );
		}
		if ( ! defined( 'TLR_ASSETS_URI' ) ) {
			define( 'TLR_ASSETS_URI', plugin_dir_url(TLR_PLUGIN_FILE) . 'assets' );
		}
	}
	...
	{
		if ( false === strpos( $class, 'Texteller' ) ) {
			return;
		}
		...
		for ( $i = $parts_count - 1; $i >= 0; $i-- ) {
			...
			if ( $parts_count - 1 === $i ) {
				...
				if ( 'interfaces' === $current ) {
					$file_name = str_ireplace( 'class-', 'interface-', $file_name );
